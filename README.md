...
- 独立模式：每名居民独立调用真实模型，只获得自己的有限感官、私人记忆和已知物品；失败保持暂停。统筹与本地模式按用户新要求使用明确标注的任务执行器，地图与文字记忆仍各自独立。
...
`npm start` 启动本机服务并输出一次性登录链接，默认本地端口为 `8787`。没有真实模型配置也能选择本地算法运行；统筹模式可按开关明确降级。独立真实模式仍需有效模型配置。
