...
export type ReplayFile={
  manifest:{schemaVersion:'1.0.0';runId:string;simCoreVersion:string;engineVersion:string;fixedDtMs:number;startTick:number;endTick:number;seed:number;configHash:string;decisionMode:'real'|'mock'|'mixed';decisionCounts:{real:number;mock:number};snapshotFiles:string[];eventLogFiles:string[];complete:boolean};
...
export class ReplayRecorder{
  ...
  capture(world:World,overlays:Record<string,SensoryOverlay>):void{
    ...
    if(last&&world.tick===last.tick)this.frames[this.frames.length-1]=frame;
      ...
      if(this.frames.length>=24000)throw Error('本轮录制已达到 20 分钟上限，请保存并开始新轮次');
  ...
  export(complete=false):ReplayFile{
    ...
    const counts={real:0,mock:0};
    for(const c of this.commits)for(const a of c.accepted){if(a.response.source==='REAL_MODEL')counts.real++;else if(a.response.source==='MOCK_TEST')counts.mock++;else throw Error('未知决策来源');}
    if(counts.real+counts.mock===0)throw Error('尚无已提交模型决策；静态场景不能作为自治演示');
    const first=this.frames[0],last=this.frames.at(-1)!;
    return copy({manifest:{schemaVersion:'1.0.0',runId:first.world.runId,simCoreVersion:SIM_VERSION,engineVersion:ENGINE_VERSION,fixedDtMs:50,startTick:first.tick,endTick:last.tick,seed:first.world.seed,configHash:CONFIG_HASH,decisionMode:counts.real&&counts.mock?'mixed':counts.real?'real':'mock',decisionCounts:counts,snapshotFiles:['embedded:frames'],eventLogFiles:['embedded:commits'],complete},frames:this.frames,commits:this.commits});
  }
}
...
export class ReplayPlayer{
  ...
  constructor(input:ReplayFile){
    ...
    for(const b of this.data.commits){
      if(!Number.isInteger(b.tick)||b.tick<m.startTick||b.tick>m.endTick)throw Error('回放决策时刻无效');
      for(const a of b.accepted){if(a.response.source==='REAL_MODEL')real++;else if(a.response.source==='MOCK_TEST')mock++;else throw Error('回放决策来源不明');}
    }
    const mode=real&&mock?'mixed':real?'real':'mock';
    if(real+mock===0||real!==m.decisionCounts.real||mock!==m.decisionCounts.mock||mode!==m.decisionMode)throw Error('回放来源计数不一致');
  }
