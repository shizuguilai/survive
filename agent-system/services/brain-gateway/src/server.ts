...
export function createGatewayServer(options:{gateway?:RealModelGateway;staticRoot?:string;loginToken?:string}={}){
  ...
  const server=createServer(async(request,response)=>{
    ...
    try{
      ...
      if(request.method==='POST'&&url.pathname==='/api/decide'){
        ...
        const result=await gateway.decide(requestBody,controller.signal);json(response,200,result);return;
      }
