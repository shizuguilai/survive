...
const sim=new Simulation({async decide(req,signal){if(++requests>48)throw Error('Real verification budget reached');return gateway.decide(req,signal);}},{world:createCampWorld(),maxRetries:0});
...
try{await sim.bootstrap();while(sim.world.tick<1600&&requests<=48&&sim.status!=='ERROR_PAUSED'){
...
const report={checkedAt:new Date().toISOString(),status:progress>0&&!freezeViolation?'passed_scoped_real_task_cycle':'partial',model:'glm-4.5-air',mockUsed:false,execution:'node-local-real-provider',requests,freezeChecks,freezeViolation,tick:sim.world.tick,taskProgress:progress,taskAcceptedBy:sim.world.camp!.tasks[0].acceptedBy,completedOperations:[...new Set(sim.world.events.filter(e=>e.kind==='action_completed').map(e=>e.text))],failedActions:[...new Set(sim.world.events.filter(e=>e.kind==='action_failed').map(e=>e.text))],inventories:sim.world.residents.map(r=>({agentId:r.id,supplies:r.supplies})),operations:[...new Set(audit.flatMap(a=>a.ops??[]))],audit,browser:'not_run',physicalDevice:'not_run',scope:'Read notice / voluntary accept / actual gather; not full camp cooperation or long-duration anti-loop guarantee'};
await writeFile('evidence/camp-real.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({status:report.status,requests,progress,freezeViolation}));
